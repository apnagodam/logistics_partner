...
class CommodityRepoImpl extends CommodityRepo{
  ...
  {
    var response = await DioInstance().post('lp_api/calculate-qv',data: commodityCalculationModel);
  ...
  Future<Map<String, dynamic>> lpQualityUpdate({required QualityUpdateModel qualityUpdateModel}) async{
   var response = await DioInstance().post('lp_api/apna_lp_quality_update',data: qualityUpdateModel);
